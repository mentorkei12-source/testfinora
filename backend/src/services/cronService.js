...
const processDailyProfits = async () => {
  ...
  try {
    // Get all active VIP users who haven't received profit today
    ...
    for (const vip of activeVips.rows) {
      ...
      // Notification (once per week to avoid spam)
    ...
    console.log(`✅ Daily profits processed for ${activeVips.rows.length} users`);
  } catch (err) {
...
const startCronJobs = () => {
  ...
  // Also run at startup for missed profits (dev mode)
